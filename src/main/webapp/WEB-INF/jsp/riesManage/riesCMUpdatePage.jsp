...
<script type="text/javascript">
	...
	function editPR(){
		console.log("进入提交方法");
		$.post("<%=request.getContextPath()%>/riesManage/riesCMReport?type=updateMessage", $('#subform').serialize() ,
		function (data){
			if(data=="success"){
				alert("修改成功");
			...
			<div class="page-content">
				<div class="panel panel-default  withoutbottom clearfix">
					<header class="panel-heading clearfix">
						<h2 class="panel-title">特约商户信息修改</h2>
					</header>
					<div class="panel-body clearfix">
						<form class="form-horizontal"  method="post"  role="form" enctype="multipart/form-data"
						 id="subform" action="">
						<input type='hidden' value ="${type}" id="type" name="type">
						<input type='hidden' value ="${companyMerchantReport.id}" id="comMerRepId" name="comMerRepId">
													...
													<select  name="Occurarea" id="Occurarea"  class="selectpicker show-tick form-control" multiple data-live-search="false">
														<c:choose>
															...
															<%-- 	<option
																	<c:if test="${mechantReport.occurarea==''}">selected="selected"</c:if>
																	value="">请选择</option>	 --%>
																<c:set value="${ fn:split(companyMerchantReport.occurarea, ',') }" var="str1" />
