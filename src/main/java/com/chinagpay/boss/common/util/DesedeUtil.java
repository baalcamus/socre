...
public class DesedeUtil {
   ...
   *          待解密数据,是Base64编码后的数据
   * @param desKey
   *          密钥 通过desKey.getBytes("UTF-8")获取密钥的字节数组
   * @return String 解密数据,通过UTF-编码转义得到最终明文
   */
//  public static String decrypt(String data, String desKey) throws Exception {
//    // 还原密钥
//    Key k = toKey(desKey.getBytes("UTF-8"));
//    /**
//     * 实例化 使用PKCS7Padding填充方式，按如下代码实现 Cipher.getInstance(CIPHER_ALGORITHM,"BC");
//     */
//    Cipher cipher = Cipher.getInstance(CIPHER_ALGORITHM);
...
//    cipher.init(Cipher.DECRYPT_MODE, k);
//    // 执行操作
//    return StringUtils.toEncodedString(cipher.doFinal(Base64.decode(data)),DsContent.DATA_CHARSET);
//  }
...
//    //加密成密文
//    byte[] desData = encrypt(data.getBytes("UTF-8"),desKey.getBytes("UTF-8"));
//    // Base64操作
//    return StringUtils.toEncodedString(Base64.encode(desData),DsContent.DATA_CHARSET);
//  }
